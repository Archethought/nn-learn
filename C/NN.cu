...
{
   //weights1[j][k][l] = syn1[j*28*100 + k*100 + l];
   //weights2[j][k] = syn2[j*10 + k];
   ...
   for (int j=0; j < 100; ++j)
   {
      for (int k=0; k < 10; ++k)
      {
         ...
         atomicAdd(&dsyn2[j*10 + k], delta2[k] * layer1[j] / (60000/10.0));
      }
   }
   //middle to input
   ...
   for (int h=0; h < 28; ++h)
   {
      for (int j=0; j < 28; ++j)
      {
         for (int k=0; k < 100; ++k)
         {
            ...
            atomicAdd(&dsyn1[h*28*100 + j*100 + k], alpha * (delta1[k] * in[i*28*28 + h*28 + j] / (60000/10.0)));
         }
      }
   }
}

__global__ void apply(float* out, float* in)
{
   int i = blockDim.x*blockIdx.x + threadIdx.x;
...
int main(int argc, char** argv)
{
   ...
   err(cudaMemcpy(d_dsyn1, dweights1, 28*28*100*sizeof(float), cudaMemcpyHostToDevice));
   err(cudaMemcpy(d_syn2, weights2, 10*100*sizeof(float), cudaMemcpyHostToDevice));
   err(cudaMemcpy(d_dsyn2, dweights2, 10*100*sizeof(float), cudaMemcpyHostToDevice));
   ...
   for (int iter=0; iter<iterations; ++iter)
   {
      ...
      apply<<<28*28, 100>>>(d_syn1, d_dsyn1);
      apply<<<10,    100>>>(d_syn2, d_dsyn2);
      //err(cudaMemcpy(dweights1, d_dsyn1, sizeof(float)*28*28*100, cudaMemcpyDeviceToHost));
      //err(cudaMemcpy(dweights2, d_dsyn2, sizeof(float)*100*10,    cudaMemcpyDeviceToHost));
