...
__global__ void LSTM1(const float* layer1i, float* layer1o, float* lstm1, const float* gate1i, const float* gate1o, float* dgate1o, const int offset)
{
...
__global__ void FpropH(float* layer1, const float* layer1o, const float* synH, const int offset)
{
...
__global__ void BpropH(const float* dlstm1, const float* gate1i, const float* gate1o, const float* dgate1o, const float* lstm1, float* dsynH, float* dsynHi, float* dsynHo, const float alpha)
{
   int i = threadIdx.x; //256
...
__global__ void DcalcH(const float* layer1i, float* dlayer1, const float* synH, const int offset)
{
   int i = threadIdx.x; //256
   int j = blockIdx.x;  //256
...
__global__ void BLSTMH(const float* layer1i, float* layer1o, const float* dlayer1, float* dlstm1, const float* lstm1, const float* gate1i, const float* gate1o, const int offset, const bool last)
{
   int i = threadIdx.x; //256
   int j = offset;
...
__global__ void Bprop1(const float* dlstm1, const float* gate1i, const float* gate1o, const float* in, float* dsyn1, float* dsyn1i, float* dsyn1o, const float alpha)
{
...
int main(int argc, char** argv)
{
   ...
   float* d_synHi;  err(cudaMalloc((void**)&d_synHi,  256*256*sizeof(float)));
   float* d_dsynHi; err(cudaMalloc((void**)&d_dsynHi, 256*256*sizeof(float)));
   float* d_synHo;  err(cudaMalloc((void**)&d_synHo,  256*256*sizeof(float)));
   float* d_dsynHo; err(cudaMalloc((void**)&d_dsynHo, 256*256*sizeof(float)));
